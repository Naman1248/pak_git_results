...
//        $data['base'] = $oClassBaseMajor->getBaseByClassIdAndBaseIdAndMajor($data['application']['cCode'], $data['application']['baseId'], $data['application']['majId'], $userData['gender']);
        ...
        if (empty($oFieldsModel->isFieldExist('district', $data['application']['cCode']))) {
        ...
        $data['admissionOffer'] = $oAdmissionOFfer->findByPK($data['application']['offerId'], 'endDate,accNo,feePurpose,cCode,className');
...
//        $obj->setCSS(ASSET_URL . 'ss/pdfForm.css');
    ...
    public function challanFormOldAction() {
...
//        if ($data['application']['cCode'] == 50 || $data['application']['cCode'] == 4) && ($data['application']['baseId']==9)
        ...
        $data['major'] = $oMajors->getMajorByOfferIdClassIdAndMajorId($data['application']['offerId'], $data['application']['cCode'], $data['application']['majId']);
        $oAdmissionOFfer = new \models\AdmissionOfferModel();
        $data['admissionOffer'] = $oAdmissionOFfer->findByPK($data['application']['offerId'], 'endDate,accNo,feePurpose,cCode,className,preReq');
        $ouserAdmissionOfferModel = new \models\cp\userAdmissionOfferModel();
        $extensionData = $ouserAdmissionOfferModel->exist($userData['userId'], $data['application']['offerId']);
        $data['admissionOffer']['endDate'] = $data['major']['endDate'];
        if (!empty($extensionData)) {
        ...
        if (strtotime($data['admissionOffer']['endDate']) < time() && $appUserId['userId'] == 101504) {
            die('expired');
        }
...
//                         print_r($appUserId);exit;
        ...
        //$oApplicationsModel = new \models\ApplicationsModel();
        $data['eduId'] = $oApplicationsModel->applicationPrerequisite($userData['userId']);
        $data['application'] = $oApplicationsModel->findByPK($get['appId'], 'offerId,majId,formNo,cCode,baseId,setNo,picExt,childBase,baseTypeDet');
        $loggedInUserId = $this->state()->get('depttUserInfo')['id'];
        if ($loggedInUserId['userId'] != 1) {
            $this->validate($data);
        }
        $oEducationModel = new \models\EducationModel();
...
//                $data['ugtResult'] = $oUGTResultModel->getPassResultByUserId($userData['userId'], $data['application']['majId']);
//                if (empty($data['gatResult']) && empty($data['ugtResult'])) {
//                    $obj->setHTML('<h1>You are not eligible to apply because of GAT Result.</h1>');
//                    $obj->browse();
//                    exit;
//                }
...
//   
            ...
            if ($loggedInUserId['userId'] != 1) {
                if (empty($data['researchObjective'])) {
                    $obj->setHTML('<h1>Please Provide Rsearch Objective Information to proceede.</h1>');
                    $obj->browse();
                    exit;
                }
            }
//            print_r($data['researchObjective']); exit;
...
//        var_dump($gatSlipData);exit;
    ...
    public function gatResultAction() {
        ...
        $oGatResultModel = new \models\gatResultModel();
        $gatResult = $oGatResultModel->findByField('userId', $userData['userId']);
//        echo '<pre>';
//        print_r($gatResult);exit;
        $obj = new \mihaka\formats\MihakaPDF();
        if (empty($gatResult)) {
...
<table width="100%">
    <tr>
        <td width="33%">Reference#:' . $userData['userId'] . '</td>
        <td width="33%" align="center">Page# {PAGENO} of {nbpg}</td>
        <td width="33%" style="text-align: right;">Printed Date: ' . date("d-m-Y") . '</td>
    </tr>
</table>');
